...
app.post('/api/mark-homework/upload', (req, res) => {
  const { imageData, model = 'gemini-2.5-pro' } = req.body;
  
  if (!imageData) {
    return res.status(400).json({ success: false, error: 'Image data is required' });
  }

  console.log(`🔍 [CLASSIFICATION] Processing image with model: ${model}`);
  ...
  const mockResponse = {
    ...
    classification: {
      isQuestionOnly: true,
      reasoning: `Test classification - using ${model}`,
      apiUsed: model === 'gemini-2.5-pro' ? 'Google Gemini 2.5 Pro' : 'OpenAI GPT-4 Omni',
  ...
  console.log(`✅ [RESPONSE] Classification: ${mockResponse.classification.apiUsed}`);
