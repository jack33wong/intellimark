...
4. is_marking_scheme (boolean): Must be true ONLY if the image is an official marking scheme or rubric containing explicit grading codes (e.g., M1, A1, B1), structural mark distributions, or evaluator notes. CRITICAL: Do NOT eject pages that contain large maps, graphs, or diagrams. You must scan the entire page. If there are question numbers (e.g., '1.1.1') or student answer spaces beneath the image, it is an active exam page and MUST be kept in the array. Do not assume a page is a marking scheme just because it lacks handwritten student work.
5. is_math_content (boolean): You must verify the academic subject of the document. This includes Mathematics, Mathematical Literacy, Physics, Chemistry, Geography, Accounting, or ANY exam paper. If the document is a piece of creative writing or clearly belongs to a non-educational subject (like a random photograph), you must set this to false. Otherwise true. CRITICAL: If you see ANY exam question numbers (e.g., 1.1, Q2) or a map/graph, you MUST set this to true.
6. rejection_reason (string): If is_acceptable_quality or is_math_content is false, provide a very short, user-friendly reason. Omit if both are true.
7. evidence_text_snippet (string): If you set is_math_content to false, provide the first 20 words as proof. Otherwise omit or leave empty.
