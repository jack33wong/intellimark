...
class AIMarkingService {
    ...
    static async generateMarkingInstructions(imageData, model, processedImage, questionDetection) {
        const compressedImage = await this.compressImage(imageData);
...
`;
    ...
    static async callGeminiForClassification(imageData, systemPrompt, userPrompt) {
        try {
    ...
    static async callOpenAIForClassification(imageData, systemPrompt, userPrompt, model) {
        try {
    ...
    static async callGeminiForMarkingInstructions(imageData, systemPrompt, userPrompt) {
        try {
            const apiKey = process.env['GEMINI_API_KEY'];
            if (!apiKey) {
                throw new Error('GEMINI_API_KEY not configured');
    ...
    static async callOpenAIForMarkingInstructions(imageData, systemPrompt, userPrompt, model) {
        try {
            const apiKey = process.env['OPENAI_API_KEY'];
            if (!apiKey) {
                throw new Error('OPENAI_API_KEY not configured');
    ...
    static generateFallbackMarkingInstructions(processedImage) {
        const annotations = [];
    ...
    static async generateChatResponse(imageData, message, model, isQuestionOnly = true) {
    ...
    static async generateContextSummary(chatHistory) {
        if (chatHistory.length === 0) {
            return '';
        }
        console.log('🔍 Generating context summary for', chatHistory.length, 'messages');
        const conversationText = chatHistory.map(item => `${item.role}: ${item.content}`).join('\n');
...
Summary:`;
    ...
    static async generateContextualResponse(message, chatHistory, model, contextSummary) {
    ...
    static async callGeminiForChatResponse(imageData, systemPrompt, userPrompt) {
        try {
            const apiKey = process.env['GEMINI_API_KEY'];
            if (!apiKey) {
                throw new Error('GEMINI_API_KEY not configured');
    ...
    static async callOpenAIForChatResponse(imageData, systemPrompt, userPrompt, model) {
        try {
            const apiKey = process.env['OPENAI_API_KEY'];
            if (!apiKey) {
                throw new Error('OPENAI_API_KEY not configured');
    ...
    static async callGeminiForTextResponse(systemPrompt, userPrompt) {
        try {
            const apiKey = process.env['GEMINI_API_KEY'];
            if (!apiKey) {
                throw new Error('GEMINI_API_KEY not configured');
    ...
    static async callOpenAIForTextResponse(systemPrompt, userPrompt, model) {
        try {
            const apiKey = process.env['OPENAI_API_KEY'];
            if (!apiKey) {
                throw new Error('OPENAI_API_KEY not configured');
    ...
    static async compressImage(imageData) {
        return imageData;
    }
}
