...
export const useDocumentDetection = (videoRef: React.RefObject<HTMLVideoElement>, isActive: boolean) => {
    ...
    useEffect(() => {
        ...
        const startLoop = () => {
            ...
            const processFrame = () => {
                ...
                // --- V36 FIX: ROBUST VIDEO CHECK ---
                // If video is null, not ready, or has 0 width, STOP.
                // This prevents the "Black Screen" bug.
                ...
                try {
                    ...
                    // --- DRAW DEBUG WINDOW (Proof of Life) ---
                    if (debugCanvasRef.current) {
                        const dCtx = debugCanvasRef.current.getContext('2d');
                        if (dCtx) {
                            // A. Draw the Video (Background)
                            dCtx.drawImage(canvas, 0, 0);

                            // B. Overlay Edges (Green)
                            const edgeImg = dCtx.createImageData(w, h);
                            for (let i = 0; i < w * h; i++) {
                                if (edges.data[i] > 0) {
                            ...
                            tC.getContext('2d')?.putImageData(edgeImg, 0, 0);
    ...
    return { detectedCorners, debugCanvasRef };
};
